...
int32 NetworkView::UpdateThread(void* data)
{
    ...
    while (!view->fTerminated) {
        ...
        while (roster.GetNextInterface(&cookie, interface) == B_OK) {
            NetworkInfo info;
            strncpy(info.name, interface.Name(), sizeof(info.name));
            ...
            strncpy(info.typeStr, typeStr.String(), sizeof(info.typeStr));
            ...
            strncpy(info.addressStr, addressStr.String(), sizeof(info.addressStr));
